...
\def\my@bx{bx}
\def\my@b{b}
\DeclareRobustCommand{\systemname}{%
  \texorpdfstring{%
    \ifx\f@series\my@bx
      DesiDiet%
    \else\ifx\f@series\my@b
      DesiDiet%
    \else
      \textsc{DesiDiet}%
    \fi\fi
...
\author[1]{Tanzim Hasan Prappo\texorpdfstring{\corref{cor1}}{}}
...
% =============================================================================
\begin{abstract}
Personalized clinical dietary decision support in South Asia faces severe public health challenges driven by the dual burden of non-communicable diseases (NCDs) and widespread micronutrient malnutrition. In Bangladesh, where NCDs account for 67\% of annual mortality, ungrounded generative large language models (LLMs) frequently hallucinate food portion sizes, omit cultural cooking staples such as added salt and cooking oil, violate critical clinical contraindications, and output culturally bizarre food combinations. Furthermore, prevailing GraphRAG methods rely on scale-invariant cosine similarity, which evaluates nutrient vector orientation rather than physical caloric and quantitative adequacy. This paper presents an artifact-grounded systems and engineering evaluation of \systemname{}, an open-source, culturally tailored clinical nutrition framework for Bangladesh. \systemname{} anchors dietary guidance in four deterministic software artifacts: (1)~a canonical knowledge graph synthesizing 582 regional foods and clinical ontologies reconciled from the Bangladesh Food Composition Table (FCT 2014), the peer-reviewed GraphRAG dataset by Dindukurthi \etal{} (\textit{Frontiers in Artificial Intelligence}, 2026), and the \textit{National Dietary Guidelines for Bangladesh 2025}, validated against thermodynamic mass-balance bounds ($95$--$105$\,g proximate sum); (2)~a culturally curated Food Compatibility Matrix (575 items, 2,328 meal-slot edges, 1,150 pairing affinities) enforcing authentic culinary roles (staple, main, side, supplementary) to eliminate bizarre combinations and deliver meaningful food suggestions; (3)~a clinical reference engine codifying national life-stage Recommended Dietary Allowances (RDA); and (4)~a portion-constrained linear optimization engine that computes exact gram weights across daily meal slots while enforcing strict caps on spices, salt ($\le 5$\,g), and cooking oil ($15$--$30$\,g). Candidate plans from bilingual LLMs are intercepted by an independent, deterministic Plan Verifier that audits ingredient-level nutrient sums, catches clinical contraindications, and executes a bounded repair loop for caloric drift exceeding 5\%. Evaluating \systemname{} across 13 cross-lingual clinical scenarios demonstrates that the system completely eliminates clinical contraindications (from 0.62 to 0.00 violations per plan), constrains caloric drift to 2.45\% ($p < 0.001$), and elevates culinary realism to 9.27/10, providing a verifiable and reproducible foundation for AI-assisted dietetics.
\end{abstract}

\begin{keyword}
clinical decision support \sep personalized nutrition \sep knowledge graph \sep portion optimization \sep food compatibility \sep deterministic verification \sep Bangladesh \sep South Asian diet
\end{keyword}
...
\begin{enumerate}[leftmargin=*,itemsep=1.5pt]
  ...
  \item \textbf{Culinary Absurdity and Bizarre Food Combinations:} Pure mathematical optimizers and generic LLMs lack culinary ontology, frequently prescribing bizarre, unpalatable food combinations (\eg{}, prescribing beef liver or heavy fish curry for breakfast, recommending raw lentils without cereal carriers, combining five uncoordinated pulses in one meal, or pairing milk tea with oily curries) that violate cultural norms and destroy patient adherence.
\end{enumerate}
...
\begin{figure*}[!t]
  ...
  \node[verif] (verifier) at (10.8, -5.8) {\textbf{Plan Verifier}\\\tiny Raw Ingredient Recalculation\\Drift Check ($>5\%$) \& Violations};
  \node[box] (output) at (6.2, -5.8) {\textbf{Verified Plan Output}\\\tiny Structured JSON + UI Delivery};
  \node[box] (refusal) at (3.2, -5.8) {\textbf{Safe Refusal}\\\tiny Emergency Medical Warning};
  ...
  \draw[arrow] (verifier) -- node[above, font=\tiny, text=green!60!black] {\textbf{Passed}} (output);
  \draw[feedback] ([xshift=-6mm]verifier.north) -- (10.2, -4.8) -- node[midway, above, font=\tiny, text=red!70!black] {Repair Drift $> 5\%$ (Max 3)} (6.6, -4.8) -- ([xshift=-4mm]planner.south);
...
\begin{itemize}[leftmargin=*,itemsep=1.5pt]
  ...
  \item \textbf{RQ2 (Algorithmic Rigor \& Culinary Realism):} Why does angular cosine retrieval fail nutritional adequacy, and how can linear portion optimization coupled with a graph-based Food Compatibility Matrix guarantee exact daily requirements while generating meaningful, authentic food suggestions?
...
\subsection{Neuro-Symbolic AI in Clinical Informatics}
To reconcile flexible conversational interaction with rigorous clinical safety, recent medical informatics has converged on \textit{Neuro-Symbolic AI}~\cite{pan2024roadmap, miao2024clinical}. Under this hybrid architectural taxonomy, neural components (such as large language models) are tasked with empathetic user communication, multilingual translation, and conversational intent parsing, while symbolic components (such as typed knowledge graphs, first-order clinical rules, and mathematical constraint solvers) strictly govern domain invariants, clinical contraindications, and physical mass balance. In clinical nutrition, where probabilistic next-token predictors are structurally incapable of exact nutrient stoichiometry and thermodynamic mass balance, \systemname{} instantiates this neuro-symbolic paradigm: the symbolic engine enforces verifiable food composition tables, polyhedral linear optimization, and independent post-generation plan verification, while the neural formatter renders the verified dietary solution into accessible, culturally resonant bilingual dialogue.
...
\subsection{Dataset Provenance \& Multi-Source Synthesis}
\label{sec:data_sources}
The nutritional and clinical foundations of \systemname{} are synthesized from four complementary authoritative sources, unifying food composition assays, cross-regional culinary structures, population-specific clinical standards, and culturally authentic meal grammars:
\begin{enumerate}[leftmargin=*,itemsep=1.5pt]
  \item \textbf{Food Composition Table for Bangladesh (FCT 2013/2014):} Formulated by the Institute of Nutrition and Food Science (INFS), University of Dhaka, and the Centre for Advanced Research in Sciences (CARS)~\cite{bdfct2014}, providing 258 core indigenous food records with authentic Bengali botanical and culinary names (\textit{lang\_bn}) and verified macronutrient/micronutrient profiles.
  \item \textbf{South Asian GraphRAG Clinical Dataset (Dindukurthi \etal{}, 2026):} Derived from the open-access clinical nutrition ontology and GraphRAG dataset by Dindukurthi~\etal{}~\cite{dindukurthi2026graphrag} (\textit{Frontiers in Artificial Intelligence}, 2026). From this source, we integrated 329 cross-regional South Asian foods culturally familiar within Bangladesh, alongside a 70-condition clinical disease ontology, 284 therapeutic requirement edges (\texttt{[:REQUIRES]}), and 112 evidence-based contraindication rules (\texttt{[:CONTRAINDICATED]}).
  \item \textbf{National Dietary Guidelines for Bangladesh 2025 (NDG 2025):} Established by the Bangladesh National Nutrition Council (BNNC) and Ministry of Health and Family Welfare~\cite{bdnac2023}, codifying life-stage Recommended Dietary Allowances (RDAs), safe macronutrient distribution ranges, and mandatory cooking medium baselines.
  \item \textbf{Culturally Curated Food Compatibility Matrix (\texttt{food\_compatibility.csv}):} Developed in consultation with Bangladeshi nutritionists, this 575-record matrix operationalizes authentic culinary structures by assigning canonical foods to designated meal slots (\textit{breakfast, lunch, dinner, snack, all}), classifying functional culinary roles (\textit{staple, main, side, supplementary, condiment}), and defining cross-food-group pairing affinities ($s \in [0.50, 1.00]$). This artifact bridges raw biochemical data and culturally coherent daily meals.
\end{enumerate}
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \addplot[ybar interval, fill=teal!60, draw=black!70, area legend] coordinates {
        (90, 2) (92, 4) (94, 9) (96, 42) (98, 185) (100, 215) 
        (102, 98) (104, 21) (106, 4) (108, 2) (110, 0)
      };
      % Thermodynamic plausibility bounds
      \draw[red!80!black, thick, dashed] (axis cs:95, 0) -- (axis cs:95, 235);
      \draw[red!80!black, thick, dashed] (axis cs:105, 0) -- (axis cs:105, 235);
      
      % Clean non-overlapping horizontal badges positioned with generous clearance
      \node[anchor=east, font=\tiny\sffamily, text=red!80!black, align=right, fill=white, fill opacity=0.95, text opacity=1, inner sep=2.5pt, rounded corners=2pt, draw=red!70!black, thin] at (axis cs:94.3, 115) {Lower Bound\\(95.0\,g)};
      \node[anchor=west, font=\tiny\sffamily, text=red!80!black, align=left, fill=white, fill opacity=0.95, text opacity=1, inner sep=2.5pt, rounded corners=2pt, draw=red!70!black, thin] at (axis cs:105.7, 115) {Upper Bound\\(105.0\,g)};
...
\subsection{Culturally Grounded Food Compatibility Matrix \& Meal Structure Ontology}
\label{sec:food_compatibility}
While food composition tables quantify raw biochemical concentrations (energy, macronutrients, minerals, and vitamins), nutritional intake is governed by cultural grammar and culinary rules. Single-objective mathematical optimizers and naive RAG retrievers routinely generate culturally absurd combinations when optimizing solely over nutrient numbers---such as prescribing beef liver or lentil curry for breakfast, recommending raw lentils without a cereal carrier, combining three distinct pulses in a single meal, or pairing milk tea with mustard oil fish curries. Such unappetizing outputs violate South Asian gastronomic traditions and cause immediate patient attrition.

To ensure that dietary plans are meaningful, palatable, and culturally coherent, we engineered a curated Food Compatibility Matrix (\texttt{datasets/food\_compatibility.csv}) comprising 575 canonical Bangladeshi food items. For each food item $f$, the matrix operationalizes four orthogonal structural dimensions:
\begin{enumerate}[leftmargin=*,itemsep=1.5pt]
  \item \textbf{Eligible Meal Slots ($\mathcal{S}_f \subseteq \{\text{breakfast}, \text{lunch}, \text{dinner}, \text{snack}, \text{all}\}$):} Defines the temporal appropriateness of each food within the South Asian daily routine. For example, plain boiled rice (\textit{bhat}) is strictly restricted to $\{\text{lunch}, \text{dinner}\}$; flattened and puffed rice (\textit{chira}, \textit{muri}) and whole-wheat flatbread (\textit{ruti}) are assigned to $\{\text{breakfast}, \text{snack}\}$; and heavy animal proteins (beef, mutton, freshwater carps) are allocated strictly to lunch and dinner.
  \item \textbf{Functional Culinary Role ($\mathcal{R}_f \in \{\text{staple}, \text{main}, \text{side}, \text{supplementary}, \text{condiment}\}$):} Categorizes every item into functional meal components:
  \begin{itemize}[leftmargin=*,itemsep=1.0pt]
    \item \textit{Staple:} Cereal/grain carrier providing the caloric baseline (\eg{}, parboiled rice, handmade ruti).
    \item \textit{Main:} Primary protein focal dish (\eg{}, \textit{Rui macher jhol}, chicken curry, egg omelet).
    \item \textit{Side:} Micronutrient and fiber-dense accompaniments (\eg{}, red lentil \textit{masur dal}, spinach \textit{palang shak bhaji}, mixed vegetable \textit{torkari}, mashed potato/eggplant \textit{bhorta}).
    \item \textit{Supplementary:} Nutritional adjuncts consumed alongside meals or as light refreshments (\eg{}, cow's milk, homemade curd/\textit{dahi}, seasonal fruits like guava, banana, or papaya).
    \item \textit{Condiment:} Thermal cooking mediums, spices, and seasonings (\eg{}, cold-pressed mustard oil, turmeric, cumin, green chili, bounded table salt).
  \end{itemize}
  \item \textbf{Bipartite Pairing Groups ($\mathcal{P}_f \subseteq \mathcal{T}_{\text{FoodGroup}}$):} Encodes culturally harmonious co-occurrence rules across food groups. For instance, freshwater fish curries pair with cereals, grain legumes, and leafy greens, but strictly forbid co-occurrence with sweet fruits or fluid dairy.
  \item \textbf{Continuous Compatibility Score ($s_f \in [0.50, 1.00]$):} Quantifies culinary affinity and universal acceptability based on dietary frequency surveys in Bangladesh, where core everyday staples receive $s=1.00$, traditional curries $s=0.90$--$0.95$, and acquired foods $s=0.50$--$0.70$.
\end{enumerate}

As detailed in \Cref{sec:kg_formalization}, this matrix is ingested into Neo4j via \texttt{backend/migrate\_food\_compatibility.py}, generating 2,328 typed \texttt{[:HAS\_MEAL\_SLOT]} edges with attributes \texttt{\{slot, role, score, notes\}} and 1,150 \texttt{[:BEST\_PAIRED\_WITH]} relationships. This relational graph provides the semantic pruning mechanism that constrains the portion optimizer to culturally authentic meal templates.

\subsection{Knowledge Graph Formalization}
\label{sec:kg_formalization}
The validated data and food compatibility relationships are ingested into a Neo4j graph database formalized as a heterogeneous directed property graph $\mathcal{G} = (\mathcal{V}, \mathcal{E}, \mathcal{T}_v, \mathcal{T}_e, \phi_v, \phi_e)$, where $\mathcal{V}$ is the entity set, $\mathcal{E}$ is the relationship set, $\mathcal{T}_v$ and $\mathcal{T}_e$ are node and edge type alphabets, and $\phi_v: \mathcal{V} \to \mathcal{T}_v$, $\phi_e: \mathcal{E} \to \mathcal{T}_e$ are mapping functions. The schema details are summarized in \Cref{tab:kg_entities} and visual traversal is illustrated in \Cref{fig:kg_subgraph}.
...
\midrule
\texttt{(:Food)} & 582 & \texttt{code}, \texttt{name\_en}, \texttt{name\_bn}, \texttt{food\_group}, \texttt{edible\_coeff} & Canonical edible items from Bangladesh FCT 2014~\cite{bdfct2014} \& Dindukurthi~\etal{}~\cite{dindukurthi2026graphrag} \\
...
\texttt{(:Disease)} & 70 & \texttt{name}, \texttt{name\_bn}, \texttt{category}, \texttt{contraindications} & Clinical non-communicable and deficiency states~\cite{dindukurthi2026graphrag} \\
...
\texttt{[:REQUIRES]} & 284 & \texttt{clinical\_priority}, \texttt{target\_tier}, \texttt{notes} & Disease-to-nutrient therapeutic requirements~\cite{dindukurthi2026graphrag} \\
\texttt{[:CONTRAINDICATED]} & 112 & \texttt{risk\_severity}, \texttt{threshold\_rule}, \texttt{clinical\_rationale} & Hard clinical exclusion rules~\cite{dindukurthi2026graphrag} (\eg{}, CKD hyperkalemia) \\
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[scale=4.5]
    % Axes
    \draw[->, thick, black!75] (0, 0) -- (1.25, 0) node[right, font=\scriptsize\sffamily] {Nutrient 1 (\% RDA)};
    \draw[->, thick, black!75] (0, 0) -- (0, 1.25) node[above, font=\scriptsize\sffamily] {Nutrient 2 (\% RDA)};
    
    % Grid & RDA Cap Box (100% envelope)
    \draw[step=0.2, gray!20, very thin] (0,0) grid (1.0, 1.0);
    \draw[black!55, dashed, thick] (0, 1.0) -- (1.0, 1.0) -- (1.0, 0);

    % Collinear Ray (dash-dotted line along ideal 1:1 ratio y = x)
    \draw[blue!60!black, thick, dashdotted] (0, 0) -- (1.03, 1.03);

    % Target Point & Label (100% RDA)
    \fill[blue!70!black] (1.0, 1.0) circle (0.65pt);
    \node[anchor=south west, font=\tiny\sffamily, text=blue!80!black, fill=white, fill opacity=0.92, text opacity=1, inner sep=1.5pt, rounded corners=1pt] at (1.02, 1.01) {Target $\mathbf{1}$ (100\% RDA)};

    % Candidate B: 90% RDA (substantial portion)
    \draw[-{Latex[scale=0.85]}, green!55!black, line width=1.4pt] (0, 0) -- (0.90, 0.90);
    \fill[green!55!black] (0.90, 0.90) circle (0.65pt);
    % Label placed cleanly to the north-west of the green dot (inside grid, completely clear of dashed boundary)
    \node[anchor=south east, font=\tiny\sffamily, text=green!55!black, fill=white, fill opacity=0.92, text opacity=1, inner sep=1.5pt, rounded corners=1pt] at (0.86, 0.92) {$\mathbf{V}_B$ (90\% RDA)};

    % Candidate A: 5% RDA (deficient portion)
    \draw[-{Latex[scale=0.85]}, red!80!black, line width=1.6pt] (0, 0) -- (0.10, 0.10);
    \fill[red!80!black] (0.10, 0.10) circle (0.65pt);
    % Pointer and label cleanly in left part of open wedge (x from 0.18 to ~0.38, y=0.15)
    \draw[red!75!black, thin] (0.10, 0.10) -- (0.18, 0.15);
    \node[anchor=west, font=\tiny\sffamily, text=red!80!black, fill=white, fill opacity=0.92, text opacity=1, inner sep=1.5pt, rounded corners=1pt] at (0.18, 0.15) {$\mathbf{V}_A$ (5\% RDA)};

    % Candidate C: High-density Spice (Cumin/Turmeric)
    \draw[-{Latex[scale=0.85]}, orange!80!black, line width=1.3pt] (0, 0) -- (1.00, 0.04);
    \fill[orange!80!black] (1.00, 0.04) circle (0.65pt);
    % Label starts at x=0.50 and extends to right (x ~0.97, y=0.06), leaving wide gap from V_A
    \node[anchor=south west, font=\tiny\sffamily, text=orange!80!black, fill=white, fill opacity=0.92, text opacity=1, inner sep=1.5pt, rounded corners=1pt] at (0.50, 0.06) {$\mathbf{V}_C$ (Spice: 100\% N1, 4\% N2)};

    % Explanatory Callout Box: compact and elegant, sitting well to the left of Target 1
    \node[draw=black!60, fill=white, rounded corners=2.5pt, align=left, font=\tiny\sffamily, inner sep=3pt, anchor=south west] at (0.04, 1.03) {
      \textbf{Cosine Scale Pathology:}\\
      $\cos(\mathbf{V}_A, \mathbf{1}) = \mathbf{1.000}$ \textit{(5\% RDA)}\\
      $\cos(\mathbf{V}_B, \mathbf{1}) = \mathbf{1.000}$ \textit{(90\% RDA)}\\
      \textit{Identical rank despite 18-fold gap.}
    };
  \end{tikzpicture}
  }
...
\paragraph{Solver Implementation and Guarantees}
To ensure computational reproducibility and real-time execution, the portion optimization problem in Eq.~\eqref{eq:portion_objective} is solved using the dual-simplex method within the SciPy HiGHS optimization suite (\texttt{scipy.optimize.linprog} configured with \texttt{method='highs-ds'}) under primal and dual feasibility tolerance thresholds of $\epsilon = 10^{-5}$. The dual-simplex algorithm guarantees deterministic, globally optimal convergence in under 90\,ms across all five daily meal slots, returning an explicit infeasibility status whenever mutually contradictory clinical bounds are encountered (as demonstrated in Section~\ref{sec:benchmarks}).
...
\subsection{Culinary Realism, Food Compatibility \& Meaningful Food Pairings}
To eliminate unrealistic optimization artifacts and guarantee meaningful, appetizing meals, \systemname{} enforces five authentic South Asian culinary rules (\Cref{fig:meal_slots}):
\begin{enumerate}[leftmargin=*,itemsep=1.5pt]
  ...
  \item \textbf{Role-Templated Candidate Selection via Compatibility Graph:}
  To prevent the linear program from generating bizarre food combinations (\eg{}, combining multiple uncoordinated lentils without a staple cereal, or pairing heavy meat curries with breakfast tea), the candidate food set $\mathcal{F}_{\text{slot}}$ for each meal slot $s$ is pruned prior to linear solving using the Food Compatibility Matrix (\Cref{sec:food_compatibility}). Candidate foods must satisfy:
  \begin{equation}
  \mathcal{F}_{\text{slot}} = \{ f \in \mathcal{F}_{\text{safe}} \mid (f, s) \in \mathcal{E}_{\text{HAS\_MEAL\_SLOT}} \}
  \end{equation}
  Furthermore, \systemname{} instantiates candidate meal slots according to authentic South Asian role templates:
  \begin{equation}
  \mathcal{F}_{\text{plan}}(s) = \{f_{\text{staple}}\} \cup \{f_{\text{main}}\} \cup \mathcal{F}_{\text{side}} \cup \mathcal{F}_{\text{supp}}
  \end{equation}
  where template components satisfy:
  \begin{align}
  |\{f_{\text{staple}}\}| &= 1, \quad f_{\text{staple}} \in \mathcal{F}_{\text{staple}} \\
  |\{f_{\text{main}}\}| &= 1, \quad f_{\text{main}} \in \mathcal{F}_{\text{main}} \\
  1 \le |\mathcal{F}_{\text{side}}| &\le 2, \quad \mathcal{F}_{\text{side}} \subset \mathcal{F}_{\text{side\_pool}}
  \end{align}
  enforcing cross-group pairing compatibility:
  \begin{equation}
  \text{Affinity}(f_{\text{main}}, f_{\text{side}}) \ge \tau_{\text{compat}} \quad (\tau_{\text{compat}} = 0.80)
  \end{equation}
  This semantic pre-structuring guarantees that the dual-simplex portion engine solves quantities strictly for dishes that form an authentic, appetizing, and culturally harmonious Bangladeshi meal.
\end{enumerate}
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      \legend{Unverified LLM Drift (\%), \textnormal{\systemname{}} Verified Drift (\%)}
    \end{axis}
  \end{tikzpicture}
...
To evaluate the architecture, we established a standardized research harness (\texttt{backend/benchmarks/}) operating over 13 multi-condition clinical scenarios across three language formats (English, native Bengali, and code-mixed \textit{Banglish}). While $N=13$ clinical manifests are presented for in-depth scenario audit, each scenario was systematically stress-tested across all three linguistic modalities ($13 \times 3 = 39$ evaluation runs), rigorously covering the full pathological spectrum of high-prevalence Bangladeshi non-communicable diseases (Type~2 diabetes, hypertension, chronic kidney disease, gestational nutrition, and iron-deficiency anemia).
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[scale=2.6]
    ...
    \node[draw=black!70, fill=white, rounded corners=2pt, align=left, font=\tiny\sffamily] at (0, -1.95) {
      ...
      \tikz\draw[green!70!black, fill=green!30] (0,0) rectangle (0.2,0.15); \textnormal{\systemname{}}
    };
  \end{tikzpicture}
  }
...
\paragraph{Culinary Realism \& Meaningful Food Suggestion Audit}
To assess whether optimized diets produce practical and meaningful meal suggestions, a clinical panel audit was conducted in consultation with a registered clinical nutritionist. Plans were evaluated across two criteria: (i)~\textit{Slot Appropriateness} (whether foods conformed to traditional breakfast, lunch, dinner, or snack patterns) and (ii)~\textit{Pairing Harmony} (whether selected mains, staples, and sides formed cohesive South Asian meals without bizarre combinations). Unconstrained LLMs and naive RAG baselines scored poorly ($4.20/10$ and $5.10/10$, \Cref{tab:benchmark_results}), exhibiting a 42.3\% error rate characterized by bizarre artifacts (prescribing heavy fish curry or mutton for breakfast, recommending raw pulses without a staple cereal carrier, combining four distinct lentils in one meal, or pairing milk tea with oily curries). In contrast, \systemname{}'s Food Compatibility Matrix achieved a score of $\mathbf{9.27/10}$ ($W = 0.0, p = 0.00048$, Cohen's $d = 4.12$), with $100\%$ slot conformance and zero culinary incoherence across all 39 evaluation runs. This confirms that mathematical portion optimization requires an explicit cultural grammar to generate meaningful, patient-acceptable dietary suggestions.
...
\begin{table}[!htbp]
\centering
\caption{Ablation Analysis of Pipeline Safety and Cultural Controls.}
\label{tab:ablation}
\footnotesize
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}Xccc}
\toprule
\textbf{Ablation Variant} & \textbf{Violations} & \textbf{Drift (\%)} & \textbf{Culinary} \\
\midrule
\textbf{Full \systemname{} Pipeline} & \textbf{0.00} & \textbf{2.45} & \textbf{9.27 / 10} \\
w/o Plan Verifier & 0.62 & 22.40 & 9.15 / 10 \\
w/o Portion Optimizer & 1.15 & 14.82 & 6.85 / 10 \\
w/o Compatibility Matrix & 0.00 & 2.45 & \textbf{5.15 / 10} \\
w/o Safety Guard & 0.23 & 2.45 & 9.25 / 10 \\
w/o Cooking Oil Model & 0.00 & 16.80 & 7.20 / 10 \\
\bottomrule
\end{tabularx}
\end{table}

\paragraph{Ablation Analysis}
\Cref{tab:ablation} reports an ablation study isolating the contribution of each safety and cultural component. Removing the Plan Verifier causes energy drift to jump from 2.45\% to 22.40\% and introduces 0.62 contraindication violations per plan. Omitting cooking oil accounting introduces a systematic 16.80\% caloric underestimation. Crucially, removing the Food Compatibility Matrix preserves mathematical caloric balance (drift: 2.45\%) but causes culinary realism to collapse from 9.27/10 to 5.15/10, demonstrating that clinical safety without cultural compatibility produces mathematically sound but practically unusable dietary plans.
...
User attrition in digital health applications frequently exceeds 75\% within 30 days due to manual data logging friction~\cite{villinger2019health}. In Bangladesh, where health literacy varies widely, requiring patients to weigh food and navigate complex English drop-down menus leads to rapid abandonment. \systemname{} addresses this by deploying across three synchronized modalities:
\begin{enumerate}[leftmargin=*,itemsep=1.5pt]
  \item \textbf{Web Clinical Dashboard:} Provides structured weekly meal schedules, macronutrient progress indicators, and micronutrient tracking against reference intake envelopes.
  \item \textbf{Native Bengali WhatsApp Interface:} Enables patients to query dietary recommendations and log meals via natural conversational Bengali without requiring the installation of standalone applications.
  \item \textbf{Deterministic Portion Verification Engine:} Displays transparent nutrient recalculations alongside validated portion allocations and contraindication-safe alternatives in real time.
\end{enumerate}
...
This paper has presented an artifact-grounded systems evaluation of \systemname{}, a culturally tailored knowledge graph and deterministic portion-planning framework for South Asian nutrition. By replacing scale-invariant cosine heuristics with an exact linear portion optimization solver, incorporating a curated 575-item Food Compatibility Matrix for culturally coherent meal structures, and introducing an independent post-generation Plan Verifier, \systemname{} resolves the critical vulnerabilities of portion hallucination, caloric drift, contraindication failure, and bizarre food pairings in generative nutrition AI. Grounded in 582 canonical Bangladeshi foods, national dietary guidelines, and authentic culinary roles, the system constrains caloric drift to 2.45\%, eliminates clinical contraindications, and elevates culinary realism to 9.27/10, establishing a reproducible foundation for safe, scalable, and meaningful dietary decision support.
