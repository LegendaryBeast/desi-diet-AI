...
% =============================================================================
\begin{abstract}
Personalized clinical dietary decision support in South Asia faces severe public health challenges driven by the dual burden of non-communicable diseases (NCDs) and widespread micronutrient malnutrition. In Bangladesh, where NCDs account for 67\% of annual mortality, ungrounded generative large language models (LLMs) frequently hallucinate food portion sizes, omit cultural cooking staples such as added salt and cooking oil, and violate critical clinical contraindications. Furthermore, prevailing GraphRAG methods rely on scale-invariant cosine similarity, which evaluates nutrient vector orientation rather than physical caloric and quantitative adequacy. This paper presents an artifact-grounded systems and engineering evaluation of \systemname{}, an open-source, culturally tailored clinical nutrition framework for Bangladesh. \systemname{} anchors dietary guidance in three deterministic software artifacts: (1)~a canonical knowledge graph reconciling 582 foods from the Bangladesh Food Composition Table (FCT 2014) validated against thermodynamic mass-balance bounds ($95$--$105$\,g proximate sum); (2)~a clinical reference engine codifying national dietary guidelines and life-stage Recommended Dietary Allowances (RDA); and (3)~a portion-constrained linear optimization engine that computes exact gram weights across daily meal slots while enforcing strict caps on spices, salt ($\le 5$\,g), and cooking oil ($15$--$30$\,g). Candidate plans from bilingual LLMs are intercepted by an independent, deterministic Plan Verifier that audits ingredient-level nutrient sums, catches clinical contraindications, and executes a bounded repair loop for caloric drift exceeding 5\%. Evaluating \systemname{} across 13 cross-lingual clinical scenarios demonstrates that the system completely eliminates clinical contraindications (from 0.62 to 0.00 violations per plan) and constrains caloric drift to 2.45\% ($p < 0.001$), providing a verifiable and reproducible foundation for AI-assisted dietetics.
\end{abstract}
