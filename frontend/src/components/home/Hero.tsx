...
export const Hero = () => {
  ...
  const badges = [
    { icon: BookOpen, label: isBn ? 'NDG 2025 অনুসারী' : 'NDG 2025 Compliant' },
    { icon: ShieldCheck, label: isBn ? 'WHO নির্দেশিকা' : 'WHO Guidelines' },
    { icon: Utensils, label: isBn ? '৩২০+ দেশীয় খাবার' : '320+ Local Foods' },
  ];

  return (
    <section className={`min-h-screen relative overflow-hidden flex flex-col justify-center bg-cream ${isBn ? 'font-bn' : ''}`} id="home">
      ...
      <div className="relative z-10 px-6 md:px-12 lg:px-24 pt-28 pb-16 max-w-[1400px] mx-auto w-full">
        ...
        {/* 2-col grid: left = badges + headline + CTA | right = video */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-10 lg:gap-16 items-start">
          ...
          <div className="flex flex-col">
            ...
            >
              <span className="text-[clamp(1rem,2.5vw,1.2rem)] font-medium text-accent block mb-3 tracking-wide">
                {isBn ? 'বাংলাদেশের প্রথম AI-চালিত পুষ্টি সহকারী' : 'First AI-Driven Nutrition Assistant for Bangladesh'}
              </span>
              <h1 className="font-body text-[clamp(3rem,7vw,6.5rem)] font-black leading-[0.9] tracking-tight text-ink">
                ...
                <span className={`block text-[clamp(1.8rem,4vw,3.5rem)] font-bold text-ink mt-2 leading-tight ${isBn ? 'font-bn' : 'font-display'}`}>
                  {isBn ? (
                    <>আপনার স্বাস্থ্য, <br className="hidden sm:block" />আপনার পরিকল্পনা।</>
                  ) : (
                    <>Your Health, <br className="hidden sm:block" />Your Personalized Plan.</>
                  )}
                </span>
              </h1>
            </motion.div>
            ...
            <motion.p
              ...
              className="mt-6 text-[0.95rem] md:text-[1.05rem] leading-[1.8] text-ink-muted max-w-[520px]"
            >
              {isBn
                ? 'ডায়াবেটিস, উচ্চরক্তচাপ, কিডনি রোগসহ ৭০+ রোগের জন্য বিশেষজ্ঞ-অনুমোদিত ডায়েট পরিকল্পনা। বাংলাদেশের জাতীয় পুষ্টি নির্দেশিকা (NDG), WHO এবং ICMR গাইডলাইন অনুসরণ করে তৈরি।'
                : 'Clinical dietary planning tailored for 70+ conditions including Diabetes, Hypertension, and CKD. Grounded in Bangladesh Dietary Guidelines (NDG), WHO, and ICMR.'}
            </motion.p>
            ...
            >
              <Link to={isLoggedIn ? '/dashboard' : '/profile'}>
                <button className="group inline-flex items-center justify-center gap-2 px-8 py-4 bg-ink text-cream rounded-2xl font-bold text-sm md:text-base hover:bg-accent transition-all duration-300 shadow-xl shadow-ink/10 active:scale-[0.98]">
              ...
              <Link to="/conditions" className="inline-flex items-center justify-center gap-1.5 px-6 py-4 border border-ink/10 rounded-2xl text-sm text-ink-muted hover:text-ink hover:border-ink/25 transition-all bg-white/60 backdrop-blur-sm">
                {isBn ? 'রোগভিত্তিক ডায়েট দেখুন' : 'Explore Clinical Conditions'}
              </Link>
            </motion.div>
            ...
            >
              ...
              <span className="ml-1">
                {isBn ? 'নির্ভরযোগ্য পুষ্টিবিদ-অনুমোদিত প্রযুক্তি' : 'Dietitian-verified clinical AI'}
              </span>
            </motion.div>
          </div>
          ...
          >
            ...
            <div className="relative flex flex-col flex-1 bg-white/80 backdrop-blur-sm rounded-[2rem] p-3 shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-ink/5 h-full">
              ...
              <div className="flex items-center gap-2 px-3 pb-3 border-b border-ink/5 mb-3 shrink-0">
                ...
                <span className="text-[0.65rem] text-ink-faint mx-auto pr-8">
                  {isBn ? 'DesiDiet — পরিচিতি ভিডিও' : 'DesiDiet — Introduction Video'}
                </span>
              </div>
              ...
              <div className="relative w-full rounded-xl overflow-hidden" style={{ paddingBottom: '56.25%' }}>
                <iframe
                  ...
                  title={isBn ? "DesiDiet পরিচিতি ভিডিও" : "DesiDiet Introduction Video"}
                  frameBorder="0"
            ...
            >
              ...
              <div className="space-y-4 mb-4">
                <div className="flex justify-end">
                  <div className="bg-ink text-cream rounded-2xl rounded-tr-sm px-4 py-3 max-w-[85%] text-[0.85rem] leading-relaxed shadow-sm">
                    {isBn
                      ? 'আমার ডায়াবেটিস আছে। আজকের জন্য ২,০০০ ক্যালোরির একটি দেশীয় ডায়েট প্ল্যান দাও।'
                      : 'I have Type-2 Diabetes. Give me a 2,000 kcal Bangladeshi meal plan for today.'}
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div className="bg-cream-dark text-ink border border-ink/5 rounded-2xl rounded-tl-sm p-4 text-[0.85rem] leading-relaxed shadow-sm w-full">
                    <div className="font-bold text-accent mb-2">
                      {isBn ? 'সকালের নাস্তা (৪০০ ক্যালোরি):' : 'Breakfast (400 kcal):'}
                    </div>
                    <ul className="space-y-1.5 mb-3 text-ink-muted">
                      <li className="flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-accent" />
                        {isBn ? 'লাল আটার রুটি - ২টি (১২০ গ্রাম)' : 'Whole wheat roti - 2 pcs (120g)'}
                      </li>
                      <li className="flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-accent" />
                        {isBn ? 'মিক্সড সবজি ভাজি - ১ বাটি (১৫০ গ্রাম)' : 'Mixed vegetable bhaji - 1 bowl (150g)'}
                      </li>
                    </ul>
                    <div className="font-bold text-accent mb-2 mt-3">
                      {isBn ? 'দুপুরের খাবার (৬০০ ক্যালোরি):' : 'Lunch (600 kcal):'}
                    </div>
                    <ul className="space-y-1.5 text-ink-muted">
                      <li className="flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-accent" />
                        {isBn ? 'লাল চালের ভাত - ১ কাপ (১৫০ গ্রাম)' : 'Parboiled rice - 1 cup (150g)'}
                      </li>
                      <li className="flex items-center gap-2">
                        <span className="w-1 h-1 rounded-full bg-accent" />
                        {isBn ? 'রুই মাছের ঝোল - ১ পিস (১০০ গ্রাম)' : 'Rui fish curry - 1 pc (100g)'}
                      </li>
                    </ul>
                  </div>
                </div>
              ...
              <div className="relative">
                <div className="w-full bg-cream rounded-full pl-5 pr-12 py-3.5 border border-ink/5 flex items-center shadow-inner">
                  <span className="text-[0.85rem] text-ink-faint">
                    {isBn ? 'আপনার প্রশ্ন লিখুন...' : 'Ask anything about your diet...'}
                  </span>
                </div>
            ...
            >
              ...
              <span className="text-[0.65rem] font-bold text-ink">
                {isBn ? 'ADA গাইডলাইন যাচাইকৃত' : 'ADA Guidelines Verified'}
              </span>
            </motion.div>
          </motion.div>
        </div>
        ...
        >
          ...
          <span className="text-[0.58rem] tracking-widest uppercase font-body">
            {isBn ? 'স্ক্রোল করুন' : 'Scroll down'}
          </span>
        </motion.div>
      </div>
    </section>
