...
export default class EventSource {
  ...
  constructor(url: string, options: any = {}) {
    ...
    if (!url || (typeof url !== 'string' && typeof (url as any).toString !== 'function')) {
      throw new SyntaxError('[EventSource] Invalid URL argument.');
    }

    if (typeof (url as any).toString === 'function') {
  ...
  open() {
    try {
      ...
      this._xhr.onerror = () => {
        ...
        this.dispatch('error', {
          type: 'error',
          message: this._xhr ? this._xhr.responseText : '',
          xhrStatus: this._xhr ? this._xhr.status : 0,
          xhrState: this._xhr ? this._xhr.readyState : 0,
        });
      };
  ...
  _getLastDoubleNewlineIndex(response: string) {
    if (!this.lineEndingCharacter) return -1;
